...
2026年7月に、RD ベースの独自記法から **GitHub Flavored Markdown（GFM）ベースの記法**に移行しました。
...
3. プリプロセッサ指令: `#%since` / `#%until` など（prefix は #285 で `#@` から `#%` に変更。旧 `#@` も受理）
...
シグネチャ見出しの**直後の行**に、kramdown の Block IAL に似た属性行を
書くことで、メソッドにメタデータを付けられます。
...
|属性|意味|
|----|----|
|`nomethod`|説明のためだけに記載されていて、実際には定義されていないメソッド（例: `Object#to_a`）。クラスページで「説明のための未定義メソッド」として区分表示される|
|`undef`|そのクラスで意図的に未定義化されているメソッド（例: `Complex` における `Comparable` の比較メソッド）。ページ生成・検索の対象外になる|
|`since="3.2"`|その名前のメソッドが Ruby 3.2 で追加されたことを明示する。署名の横に「Ruby 3.2 から」バッジが表示される|
|`until="4.0"`|その名前のメソッドが Ruby 4.0 で削除された（4.0 では使えない）ことを明示する。署名の横に「Ruby 4.0 で削除」バッジが表示される|
|`since=""`|初出バージョンが不明（記録が残る最古のバージョンより前から存在する）ことを明示し、**バッジを表示しない**。メソッド自体は昔からあるのに記載が後から追加され、自動算出が記載時期を初出として誤表示する場合の抑止用（例: `Array#collect` は 2.4.0 のドキュメント凍結後に記載が追加されたため「Ruby 2.5.0 から」と誤算出されていた）|

* 属性行は**直前のシグネチャ見出し行のみ**に結び付きます（kramdown の Block IAL と
  同じ解釈）。別名（複数シグネチャ）のエントリで `nomethod` / `undef` を使う場合は、
  すべてのシグネチャ見出しの直後にそれぞれ付けてください（一部にだけ付いていると
  ビルドエラーになります）
* `since` / `until` は逆に**シグネチャごとに別の値を付けられます**。別名の一方だけが
  後から追加された場合は、そのシグネチャ見出しの直後にだけ付けてください
...
* シグネチャ見出し行と属性行が連続している範囲がひとつのエントリ（別名グループ）に
  なります。属性行の直後に別エントリを続ける場合は空行で区切ってください
* 未知の属性はビルドエラーになります（typo 検出のため）。バージョンの値は
  二重引用符の中に数字とドットで書きます（例: `since="1.9.3"`）。
  空値（`since=""`）は上記の「不明」の明示です
* 通常、メソッドの追加・削除バージョンは全バージョンの DB から**自動算出**されるので
  明示は不要です。`since` / `until` を書くのは、自動算出が届かない場合
  （Ruby 1.8.7 以前から存在するメソッドに正確な版を示したい場合）や、算出値を
  補正したい場合だけです。明示した値は自動算出より優先されます
...
prefix は bitclust#285 により `#@` から `#%` に変更されました（`#%` は Markdown の
見出しと解釈が衝突せず、GitHub 上で `@since` などがユーザー mention として
誤解釈されないため）。指令の意味自体は変更なく、旧 prefix `#@` も後方互換で
引き続き受理されます。
...
`#%since` / `#%until` / `#%version` / `#%if` は `#%else` と組み合わせられます。
