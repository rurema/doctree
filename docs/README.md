...
| [MailingList](MailingList.md) | アーカイブ | ML は QuickML の仕様で消滅済みと明記されている。歴史的記録 |
...
| ページ | 提案 | 理由・備考 |
|--------|------|-----------|
| [BitClust](BitClust.md) | 移動 | bitclust の使い方。ツールのリポジトリ（bitclust/doc/）に置くのが自然。コマンド例の鮮度確認も |
| [BitClustDatabase](BitClustDatabase.md) | 移動 | DB 内部構造の解説。同上。内容が現行実装と合っているか要確認 |
...
| [ReleasePackageHowTo](ReleasePackageHowTo.md) | アーカイブ | パッケージ版（refe2 同梱）の使い方（2017年）。現在の配布実態と乖離 |
| [ReleasedProducts](ReleasedProducts.md) | 部分更新 | 配布物一覧。doc.okkez.net 等の古いリンクを整理し、docs.ruby-lang.org 中心に |
| [ReleaseProcess](ReleaseProcess.md) | 削除 | 空ページ（TODO のまま） |

### 歴史文書（アーカイブ）

いずれも初期フェーズ（〜2010年代前半）の計画・体制の記録。内容は現体制と乖離しているが、
プロジェクト史として価値があるため `docs/archive/` に移して凍結する。

| ページ | 備考 |
|--------|------|
| [Phase1WorkingScheme](Phase1WorkingScheme.md) | 第一段階の進め方 |
| [Phase2WorkingScheme](Phase2WorkingScheme.md) | 第二段階の進め方（381行） |
| [Phase3WorkingScheme](Phase3WorkingScheme.md) | 第三段階の進め方 |
| [Phase3ReviewProcess1](Phase3ReviewProcess1.md) | 第三段階のレビュー手順 |
| [ProjectTimeLine](ProjectTimeLine.md) | 〜2018年の年表。むしろ追記して維持する選択肢も |
| [WorkingProcess](WorkingProcess.md) | 作業マニュアルへのリンク集（リンク先の大半がアーカイブ対象） |
| [MaintenancePhase](MaintenancePhase.md) | メンテナンスフェイズ構想。Subversion・ファイル別メンテナ制前提で現体制と別物 |
| [DischargingProcess](DischargingProcess.md) | 担当交代の手順。ASSIGN ファイル前提の旧体制 |
...
| [ProjectGoal](ProjectGoal.md) | 削除 | 空ページ。目標は CONTRIBUTING.md に記載済み |
| [Committers](Committers.md) | 削除 | 空ページ |
| [License](License.md) | 部分更新 | リンク2行。`refm/doc/license.rd` → `manual/doc/license.md` に更新 |
| [SitePolicy](SitePolicy.md) | 削除 | wiki の編集権限ポリシー。リポジトリ移行後は PR ベースになるため不要 |
| [ThirdPartyTools](ThirdPartyTools.md) | 維持 | 外部ツール事例集。リンク切れ確認は任意（rurema-search の行はサービス終了を反映） |
| [HowToInstallRubys](HowToInstallRubys.md) | 削除 | 複数 Ruby の環境構築（2017年、rbenv/古い Windows 前提）。現在は外部資料が充実しており、本リポジトリの検証環境も mise.toml で完結 |
| [HowToInstallRubysUnixRbenv](HowToInstallRubysUnixRbenv.md) | 削除 | 同上 |
| [HowToInstallRubysWindows](HowToInstallRubysWindows.md) | 削除 | 同上 |

## 今後の手順

1. この一覧の内容について合意を取る（この PR）
2. 削除・アーカイブ・bitclust への移動を実施
3. 残すページの wiki リンク（`[[PageName]]` 形式、全体で約100箇所）を相対リンクに変換し、
   区分に応じて書き直し・更新
