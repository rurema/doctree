...
- [ReferenceManualFormatDigest](ReferenceManualFormatDigest.md) — **書式のまとめ（まずこれ）**
- [ClassReferenceManualFormat](ClassReferenceManualFormat.md) — ファイル構造（front matter・H1/H2）
...
- [DirectoryTree](DirectoryTree.md) — doctree のディレクトリ構造

### 参加・プロジェクト運営

- [HowToJoin](HowToJoin.md) — プロジェクトに参加するには
- [Communication-and-knowledge-sharing](Communication-and-knowledge-sharing.md) — 連絡・情報共有の場
...
貢献の入口は [CONTRIBUTING.md](../CONTRIBUTING.md) です。
記法の正式仕様は bitclust の
[MARKUP_SPEC.md](https://github.com/rurema/bitclust/blob/master/doc/markdown-samples/MARKUP_SPEC.md)、
BitClust 自体の文書は
[bitclust/doc/](https://github.com/rurema/bitclust/tree/master/doc) にあります。

## 棚卸し一覧（2026-07、完了記録）

wiki 取り込み時の全40ページの棚卸しの記録です。**表の処理はすべて実施済み**です
（表中に「移動先で」「リリース時に」と記した後続作業を除く）。
...
| HowToContribute | 統合 ✅ | 内容は CONTRIBUTING.md と重複。差分（サンプル検証などの貢献形態）を CONTRIBUTING.md に移して削除 |
| HowToReport | 統合 ✅ | Issue 登録手順8行。CONTRIBUTING.md に一文で足りる |
...
| ProjectResources | 統合 ✅ | ML 前提の古い内容。生きている情報（サイト URL・リポジトリ）は README/CONTRIBUTING.md にあるため実質不要 |
...
| ページ | 提案 | 理由・備考 |
|--------|------|-----------|
| Home | 統合 ✅ | wiki のトップページ。この README が後継となるため削除 |
...
| HowToInstallRubys | 削除 ✅ | 複数 Ruby の環境構築（2017年、rbenv/古い Windows 前提）。現在は外部資料が充実しており、ビルドも最近の Ruby が1つあれば足りる |
...
3. ~~残すページの wiki リンクを相対リンクに変換し、区分に応じて書き直し・更新~~（✅）
