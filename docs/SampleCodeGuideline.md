...
* 同じ内容を複数の書き方で表現できる場合、必要な知識が最少で、かつ記述量が
  同程度であれば、より一般的でよく知られた表現を選んでください。
  例えば、リテラルで簡潔に書けるオブジェクトは、そのオブジェクトの生成方法
  自体を説明する例でない限り、リテラルで書いてください。
  ...
  | `Complex(1, 3)` | `1+3i` |
  | `Regexp.new('\d+')` | `/\d+/` |
  | `Hash[:morning, 3, :evening, 4]` | `{morning: 3, evening: 4}` |
  | `Range.new(3, nil)` | `3..` |

  ただし、書き換えによって前提知識や記述量がかえって増える場合は、無理に
  一般的な表現へ揃える必要はありません。判断に迷う場合は、Pull Request を
  送って相談してください。
...
### 例外が発生する場合
値ではなく例外が発生することを示したい場合は、`# =>` と区別するために `# ~>` を使ってください。
`# ~>` の後は、例外クラス名や例外メッセージ、実行結果の出力など、状況に応じた任意の形式で構いません。
例外は式の評価中に発生し、その式の返り値は得られないため、例外が発生する式に `p` は付けないでください。
...
* エンコーディングを指定するマジックコメントは、UTF-8 が既定なので原則として書きません。
  マジックコメント自体が例の題材である場合や UTF-8 以外の指定が必要な場合だけ
  `# encoding: UTF-8` の形式で書いてください（`-*-` で囲まない・`#` の後ろに空白・
  `:` の後ろにだけ空白・`coding` ではなく `encoding`・エンコーディング名は
  `Encoding#to_s` の結果に合わせる。`CP932` のみ定数名を使う）
