...
生成にはbitclustコマンドを使い、単一のファイルだけ生成するコマンドを実行するか、全体を見るためにまずはDBを作ってサーバを立てるかのどちらかをつかってプレビューしてください。

typoの修正など日本語の修正程度であれば以下のどちらも行わずともpull requestを送っていただいて問題ありません。
[[ReferenceManualFormatDigest]] にあるような書式も修正の対象となる場合はどちらかの方法で確認後にpull requestを送っていただけるとありがたいです。

ただし、環境が整えられなくて困っている場合はissue/pull requestのどちらかを「動作確認がまだ」である事を明記して発行していただいければ問題ありません。
