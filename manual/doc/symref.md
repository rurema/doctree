...
- **`xxx&.yyy`**:
  ...
  safe navigation operator（通称「ぼっち演算子」）。`xxx` が `nil` でないときにメソッド `yyy` を呼び出す。[d:spec/call] を参照。
...
- **`Set[1, 2] | Set[2, 3]`**:
  ...
  和集合を求めるメソッド([m:Set#|] など)。ビット演算以外の意味で
  定義されることもあります。
...
- **`-"abc"`**:
  ...
  数値では符号の反転([m:Integer#-@] など)ですが、それ以外の意味で
  定義されることもあります(例: [m:String#-@] は freeze された文字列を
  返します)。メソッド定義での表記は [ref:at] の `def -@` の項を参照。
...
- **`def xxx(**yy)`**:
  ...
  まとめて受け取る。[ref:d:spec/def#method] を参照。

- **`foo(**hash)`**:

  メソッド呼び出しにおけるハッシュのキーワード引数への展開。ハッシュの各要素を
  キーワード引数として渡す。[d:spec/call] を参照。
...
- **`foo(*ary)`**:
  ...
  個々の引数として渡す。[d:spec/call] を参照。

- **`[*3..7]`**:

  配列式の中での展開。展開した要素を含む配列を生成する。
  [ref:d:spec/literal#array] を参照。
...
- **`ary = *3..7`**:
  ...
  [ref:d:spec/operator#multiassign] を参照。
...
- **`Pathname("/usr") / "local"`**:
  ...
  パス名の連結([m:Pathname#/])など、割り算以外の意味で定義されることもあります。
...
- **`:"foo bar"`**:
  ...
  シンボルを書けます。[ref:d:spec/literal#symbol] を参照。
...
- **`a === 12`**:
  ...
  適宜再定義されます」。たとえば、[m:Range#===] は `obj` が範囲内に含まれている時に真を返します。
...
- **`in パターン => 変数`**:
  ...
  [ref:d:spec/pattern_matching#variable_binding] を参照。
