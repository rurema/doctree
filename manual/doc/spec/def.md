...
これは、クラスの特異クラスが、スーパークラスの特異クラスのサブクラス
として作られるためです。この仕組みによって、クラスの特異メソッド(クラ
スメソッド)がサブクラスにも継承されます。
...
は、通常の識別子の他、`?` や `!` の付いたものや[ref:operator]で
説明する再定義可能な演算子も含むメソッド名そのものか、
[ref:d:spec/literal#symbol] を指定します(obj.method のような式
を書くことはできません)。alias の引数はメソッド呼び出し等の一切
の評価は行われません。
...
([ref:d:spec/variables#builtin] を参照)を再定義すると動作に
支障を来す場合があります。
...
メソッドの定義を取り消します。メソッド名には、通常の識別子の
他、`?` や `!` の付いたものや[ref:operator]で説明する再定義可能
な演算子も含むメソッド名そのものか [ref:d:spec/literal#symbol] を
指定します(obj.method のような式を書くことはできません)。undef
