...
- **`(?= )`**:
  ...
  re = /((?=\d{2,4}3)\d{8})/
  p re =~ "asdf1234567890"  # => 4
  p $&                      # => "12345678"
  p re =~ "asdf234567890"   # => nil
  p re =~ "asdf1111234567890" # => 5
  p $&                      # => "11123456"
  p re =~ "asdf111112345xxxx" # => nil
  ```
  ```text
          re1(?=re2)
  ```
...
- **`(?<= )`**:
  ...
  ```text
          (?<=re)
  ```
  という表現は、「直前に re にマッチする文字列が存在する位置」を表します。
  ```ruby
  ...
  ```text
          (?<=re1)re2
  ```
  という表現は、「直前に re1 にマッチする文字列が存在する位置から始まり、re2 にマッチする文字列のパターン」を表します。
  ```ruby
  ...
  ```ruby
          ...
          p 'abcbc'.gsub(/(?<=a)b(?=c)/, '*')  # => 'a*cbc'
  ```
  戻り読みの式は固定文字長でなければなりません。
  ```text
...
- **`(?<! )`**:
  ...
  否定戻り読み(negative lookbehind)。
  パターンの否定による位置指定(幅を持たない)。

  ```text
          (?<!re)
  ```
  という表現は、「直前に re にマッチする文字列が存在しない位置」にマッチします。
  ```ruby
          # 文字列の先頭はその直前が \d にマッチすることはないので、必ず先頭にマッチする。
          p /(?<!\d)/ =~ '123' # => 0
          p /(?<!\d)/ =~ ''    # => 0
  ```
  ```text
          (?<!re1)re2
  ```
  という表現は、「直前に re1 にマッチする文字列が存在しない位置から始まり、re2 にマッチする文字列のパターン」を表します。
  ```ruby
          p /(?<!foo)bar/ =~ 'foobarbazbarfoo'  # => 9
          p $&    # => "bar"   (foo の部分の情報はない)
  ```
  否定戻り読みの式は固定文字長でなければなりません。
  ```text
          /(?<!\d+)/ # => invalid pattern in look-behind: /(?<!\d+)/
  ```
