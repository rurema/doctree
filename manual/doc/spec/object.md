...
モジュールはクラスと同様にメソッドや定数の集まりを定義しますが、
クラスと違ってインスタンスを生成できず、単独では継承関係を持ちません。

モジュールをクラスに include すると、そのモジュールのインスタンスメソッドを
クラスに定義されているかのように扱えるようになります。この仕組みを Mix-in と呼びます。
