...
以下の記法を用いて、
部分式呼び出しでの上のレベル、下のレベルでのマッチにアクセスできます。levelで呼出しのネストレベルを、nで位置を指定できます。
  - \k<n+level>     (n >= 1)
  - \k<n-level>     (n >= 1)
  - \k'n+level'     (n >= 1)
  - \k'n-level'     (n >= 1)
...
部分正規表現にのみ適用できます。on、offにはi,m,xのいずれか
...
例えば (?~abc) は "", "ab", "aab", "abb", "ccdd" などの**文字列全体**には
マッチしますが、"abc", "aabc", "ccabcdd" などの**文字列全体**にはマッチし
ません。(?~式) はあくまで通常の正規表現なので、検索対象の文字列の一部分
にはマッチすることがある点に注意してください。実際、"abc" を含む文字列
であっても、"abc" を含まない部分文字列が見つかればそこにマッチします。
