...
ローカル変数スコープ（クラス、モジュール、メソッド定義の本体）における小文字で始まる識別子への最初
の代入はそのスコープに属するローカル変数の宣言になります。
...
このように 「`$` + 特殊文字一文字」、または「`$` + 10進数字列」という名前を持つ変数を特殊変数と呼びます。
...
Unicode 以外のときは小文字に変換できる文字から始まる識別子が定数です。
