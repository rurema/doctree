...
### const SIGN_NEGATIVE_FINITE -> Integer

負の値に対応する [m:BigDecimal#sign] の値を返します。

### const SIGN_NEGATIVE_INFINITE -> Integer

負の無限大に対応する [m:BigDecimal#sign] の値を返します。

### const SIGN_NEGATIVE_ZERO -> Integer

負の 0 に対応する [m:BigDecimal#sign] の値を返します。

### const SIGN_NaN -> Integer

NaN に対応する [m:BigDecimal#sign] の値を返します。

### const SIGN_POSITIVE_FINITE -> Integer

正の値に対応する [m:BigDecimal#sign] の値を返します。

### const SIGN_POSITIVE_INFINITE -> Integer

正の無限大に対応する [m:BigDecimal#sign] の値を返します。

### const SIGN_POSITIVE_ZERO -> Integer

正の 0 に対応する [m:BigDecimal#sign] の値を返します。

### const INFINITY -> BigDecimal
...
クラスメソッド [BigDecimal.mode](m:BigDecimal.mode)([m:BigDecimal::ROUND_MODE],flag) で指定した
...
[BigDecimal.mode](m:BigDecimal.mode)([m:BigDecimal::ROUND_MODE],flag) で何も指定せず、かつ、引数を指定しない場合は「小数点以下第一位の数を四捨五入して整数(BigDecimal 値)」にします。
