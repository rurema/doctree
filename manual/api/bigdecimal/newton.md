...
[c:RuntimeError] が発生する場合があります。実行後は引数 x は破壊的に変更されます。x から解を取得します。
...
### module_function def norm(fv, zero = 0.0) -> Float
...
- **param** `x` -- 探索を開始する点を数値の配列で指定します。解が複数ある場合、初期値によって得られる解が異なります。また、初期値によっては収束せずに [c:RuntimeError] が発生する場合があります。実行後は引数 x に指定したオブジェクトに解が代入されます。
         実行後は解を表す値が代入されています。
