...
data に nil を渡すと [m:OpenSSL::PKCS7#data] で得られる文字列を用います。通常は nil を渡してください。
...
### def signers -> [OpenSSL::PKCS7::SignerInfo]

メッセージの署名者を表す [c:OpenSSL::PKCS7::SignerInfo] オブジェクトの配列を返します。

これはメッセージを署名した場合にのみ意味があります。

### def add_recipient(recipient) -> self
...
[m:OpenSSL::PKCS7.sign] で [m:OpenSSL::PKCS7::NOCERTS] を渡した場合には含まれていないので、明示的に渡す必要があります。
...
nil を渡すと [m:OpenSSL::PKCS7#data] で得られる文字列を用います。通常は nil を渡すべきです。
...
検証に失敗した場合は [m:OpenSSL::PKCS7#error_string] に失敗した理由を表す文字列がセットされます。
...
[m:OpenSSL::PKCS7#verify] で検証をした場合のみ更新されます。
