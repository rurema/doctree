...
### const NOCHECKS -> Integer
署名の検証における追加的なチェックを省略することを意味します。

### const NODELEGATED -> Integer
...
### const RESPONSE_STATUS_INTERNALERROR -> Integer
...
### const REVOKED_STATUS_CACOMPROMISE -> Integer
[m:OpenSSL::OCSP::BasicResponse#status] の失効理由コードで、
CA が危殆化したことを意味します。
...
### const REVOKED_STATUS_NOSTATUS -> Integer
[m:OpenSSL::OCSP::BasicResponse#status] の失効理由コードで、
BasicResponse にコードが含まれていないことを意味します。


### const REVOKED_STATUS_REMOVEFROMCRL -> Integer
[m:OpenSSL::OCSP::BasicResponse#status] の失効理由コードで、
CRL からの削除を意味します。
...
### def copy_nonce(request) -> Integer
...
## Class Methods
#@until 1.9.2
### def new(subject, issuer) -> OpenSSL::OCSP::CertificateId
#@else
