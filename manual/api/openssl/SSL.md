...
### const OP_ALL -> Integer

SSL 通信で各種のバグ回避コードをすべて有効にするフラグです。

[m:OpenSSL::SSL::SSLContext#options=] で利用します。

### const OP_CIPHER_SERVER_PREFERENCE -> Integer
...
### const OP_EPHEMERAL_RSA -> Integer

RSA を使う場合に常に一時的 RSA 鍵を使うことを意味するフラグです。

[m:OpenSSL::SSL::SSLContext#options=] で利用します。

### const OP_NETSCAPE_CA_DN_BUG -> Integer
...
ハンドシェイクの結果は [m:OpenSSL::SSL::SSLSocket#verify_result] で取得できます。
...
[m:OpenSSL::SSL::VERIFY_FAIL_IF_NO_PEER_CERT] と同時に指定した場合はクライアントが要求を無視した場合にはハンドシェイクを即座に停止します。
