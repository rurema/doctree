...
最後に [m:OpenSSL::X509::Store#verify] を呼び、検証したときのエラー文字列を返します。
...
最後に [m:OpenSSL::X509::Store#verify] を呼び、検証したときのエラーコードを返します。
...
[m:OpenSSL::X509::StoreContext.new] で証明書ストアコンテキストを生成する場合にはそのコンテキストにフラグがコピーされます。
...
最後に [m:OpenSSL::X509::Store#verify] を呼び、検証したときのルート CA までの証明書のチェインを返します。
...
[m:OpenSSL::X509::StoreContext.new] で証明書ストアコンテキストを生成する場合にはそのコンテキストにこの値がコピーされます。
