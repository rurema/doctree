...
### def ca_certs -> [OpenSSL::X509::Certificate]

自身が保持しているCA証明書を配列で返します。

### def certificate -> OpenSSL::X509::Certificate
...
- **param** `md_type` -- 使用するハッシュアルゴリズムを表す文字列もしくは [c:OpenSSL::Digest] のオブジェクト。省略した場合は既定のアルゴリズムが使われます
