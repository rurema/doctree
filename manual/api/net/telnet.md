...
すべて利用できます([c:SimpleDelegator] を用いて
います)。
...
### def new(opts) -> Net::Telnet
### def new(opts){|message| ... } -> Net::Telnet
...
### def login(opts, password=nil) -> String
### def login(opts, password=nil){|mesg| ...}  -> String
...
### def waitfor(opt) -> String|nil
### def waitfor(opt){|buf| ...} -> String|nil
...
opts がハッシュである場合には、送る文字列とオプションを指定できます。
その場合には以下の
