...
### def fetch_path(uri, mtime = nil, head = false) -> String | Hash
...
### def connection_for(uri) -> Net::HTTP
...
### def open_uri_or_path(uri, last_modified = nil, head = false, depth = 0) -> StringIO | File
...
### def file_uri?(uri) -> bool

"file://" で始まる文字列である場合は真を返します。そうでない場合は偽を返します。

- **param** `uri` -- URI を表す文字列を指定します。

### def get_file_uri_path(uri) -> String
