...
拡張子が .f90、.F90、.f95、.F95 のファイルを解析する事ができます。解析のためには、Fortran95 の仕様に適合している必要があります。

[注意] rdoc 2.4.0 から rdoc-f95 に分かれたため、Ruby 1.9.2 から
[lib:rdoc/parser/f95] は標準添付ライブラリに含まれなくなりました。
Ruby 1.9.2 以降でも使用したい場合は rdoc-f95 を RubyGems でインストールしてください。
...
基本的な規則は Ruby のソースコード中にドキュメントを記述する場合と同じです。ただし、Fortran95 では、コメントを記述するためには「#」ではなく、「!」を行頭に記述しなければなりません。コメントは文の後(もしくは下)に記述します。
...
### const COMMENTS_ARE_UPPER -> false

ライブラリの内部で使用します。

### const INTERNAL_ALIAS_MES -> "Alias for"

ライブラリの内部で使用します。

### const EXTERNAL_ALIAS_MES -> "The entity is"
