...
ただし、[キー, 値] という2要素の配列を要素とする配列を1つだけ渡すこともでき、その場合は各要素がキーと値の組として扱われます((3)(4)の例を参照)。

このメソッドでは生成するハッシュにデフォルト値を指定することはできません。
[m:Hash.new]を使うか、[m:Hash#default=]で後から指定してください。

- **param** `key_and_value` -- 生成するハッシュのキーと値の組です。必ず偶数個(0を含む)指定しなければいけません。ただし、[キー, 値] の配列を要素とする配列を1つだけ渡すことも可能です。
- **raise** `ArgumentError` -- 奇数個の引数を与えたときに発生します。[キー, 値] の配列を要素とする配列を1つだけ渡す形式の場合、各要素が配列でなかったり要素数が3個以上であったりすると発生します。
...
Ruby 3.2 以降ではデフォルト値に関わらず nil を返すよう変更されています。
...
デフォルト値と値としての nil を区別する必要がある場合は [m:Hash#fetch] または [m:Hash#key?] を使ってください。
...
ブロック付きの場合 self を、無しで呼ばれた場合 [c:Enumerator] を返します。
...
一方、`&` を使って lambda や [c:Method] オブジェクトをブロックとして渡す場合はこの自動分解が行われません。そのため、2引数の lambda（`->(k, v) { ... }`）を渡すコードは、
...
反復の際の評価順序はキーが追加された順です。
ブロック付きの場合selfを、無しで呼ばれた場合[c:Enumerator]を返します。
