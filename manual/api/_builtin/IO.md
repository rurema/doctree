...
現在は [m:IO.read]・`File.read` のいずれでも、"|" で始まる path は通常のファイル名として
扱われます(該当するファイルが無ければ [c:Errno::ENOENT] が発生します)。

```ruby title="例"
p IO.read("|echo hello")    # => Errno::ENOENT ("|echo hello" という名前のファイルを探そうとする)
p File.read("|echo hello")  # => Errno::ENOENT
```
#@else
[m:Kernel?.open] と同様 path の先頭が "|" ならば、"|" に続くコマンドの出力を読み取ります。
