...
Math モジュールにはさまざまな数学関数がモジュール関数として定義されています。
...
`x` が 0 に近いとき、[m:Math?.exp] の結果から 1 を引くと桁落ちによって精度が失われますが、このメソッドはそれを避けて計算します。
...
`x` が 0 に近いとき、1 に `x` を足してから [m:Math?.log] を取ると桁落ちによって精度が失われますが、このメソッドはそれを避けて計算します。
...
数学関数（[c:Math] のモジュール関数）に与えた引数が定義域に含まれていない場合に発生します。
