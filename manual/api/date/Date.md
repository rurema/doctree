...
`Date` は時刻の情報を含みません。時刻を含んだ日付は [c:Time] で表せます。
...
もっとも本実装で一番に尊重されているのは、伊国の改暦日であり、多くの場合、改暦日の既定値は [m:Date::ITALY] です。
...
このメソッドは [m:Date.httpdate] と似ていますが、日付オブジェクトを生成せずに、見いだした要素をハッシュで返します。
...
このメソッドは [m:Date.iso8601] と似ていますが、日付オブジェクトを生成せずに、見いだした要素をハッシュで返します。
...
このメソッドは [m:Date.jisx0301] と似ていますが、日付オブジェクトを生成せずに、見いだした要素をハッシュで返します。
...
このメソッドは [m:Date.parse] と似ていますが、日付オブジェクトを生成せずに、見いだした要素をハッシュで返します。
...
このメソッドは [m:Date.rfc2822] と似ていますが、日付オブジェクトを生成せずに、見いだした要素をハッシュで返します。
...
このメソッドは [m:Date.rfc3339] と似ていますが、日付オブジェクトを生成せずに、見いだした要素をハッシュで返します。
...
このメソッドは [m:Date.strptime] と似ていますが、日付オブジェクトを生成せずに、見いだした要素をハッシュで返します。
...
このメソッドは [m:Date.xmlschema] と似ていますが、日付オブジェクトを生成せずに、見いだした要素をハッシュで返します。
