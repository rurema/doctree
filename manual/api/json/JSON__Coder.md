...
options には、JSON 形式の文字列を生成する際([m:JSON?.generate])とパースする際([m:JSON?.parse])の両方に使われるオプションをハッシュで指定します。
...
- **raise** `JSON::GeneratorError` -- object(またはその内部の値)が JSON にネイティブ対応しておらず、[m:JSON::Coder.new] にブロックも指定されていなかった場合に発生します。
