...
[m:Etc?.getgrent] によって開始された /etc/group ファイルを読むプロセスを終了させファイルを閉じます。
...
[m:Etc?.getpwent] によって開始された /etc/passwdファイルを読むプロセスを終了させファイルを閉じます。
...
このメソッドが失敗した場合は [m:Etc?.getpwuid] にフォールバックするとよいでしょう。
