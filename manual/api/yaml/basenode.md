...
引数で指定したキーに対応する値(もしくは引数で指定したインデックスに対応する値の配列)を返します。対応する値がない場合は nil を返します。

- **param** `key` -- [c:Hash] のキーか [c:Array] のインデックスを文字列で指定します。後者の場合は複数個指定する事ができます。

### def at(segment) -> object | nil

指定したキー(もしくはインデックス)に対応する値を返します。対応する値がない場合は nil を返します。

- **param** `segment` -- [c:Hash] のキーか [c:Array] のインデックスを文字列で指定します。
...
[m:Syck::Node#transform] で Ruby のオブジェクトにした配列を返します。
