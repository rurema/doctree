...
: blade
  ml archive (((<(('blade/ruby'))|URL:http://blade.nagaokaut.ac.jp/ruby/>)))
...
: Coerce
  ...
  いインスタンスが引数として渡された時には(({coerce}))
  メソッドを使って変換を行うように取り決められている。
  (({coerce}))メソッドは引数として与えられた値(を変換し
...
: Eiffel
  ...
  Eiffelに似ていない。似ているのはブロックが(({end}))で
  終るところと、(({rescue}))という予約語だけか。
...
: Frequently Asked Questions
  よくある質問とその答え集。
  ((<Ruby FAQ>))はまだまだ発展途上である。質問と答えは随時募集中。
...
: (({main}))
  ((<Ruby用語集/トップレベル>))での((<Ruby用語集/self>))。
  (({self}))がないわけにはいかないので、ただそこにある
  ためだけの単なる(({Object}))クラスのインスタンスであるが、
...
: RAA
  Ruby Application Archive(((<RAA|URL:http://www.ruby-lang.org/en/raa.html>)))
...
: Sather
  ...
  ((<Ruby用語集/Eiffel>))よりもSatherが好きだ。しかし、
  SatherもやっぱりRubyには全然似ていない。

: (({self}))
  ((<Ruby用語集/レシーバ>))を参照する式。なぜ
  (({self}))かというと、((<Ruby用語集/メソッド>))
  を動詞と考えるとレシーバは主語に当たり、メソッドから見ると
  ((*自分*))であるからという説があるが、Rubyでは深
  く考えず、単に((<Ruby用語集/Smalltalk>))を真似ただけ
  だ、という説が有力である。

: Smalltalk
  オブジェクト指向プログラミング言語。現在の
  ((<Ruby用語集/オブジェクト指向>))と
  いう((<Ruby用語集/パラダイム>))の基礎を築いた言語。
...
: (({undef}))
  メソッドを未定義状態にすること。((<Ruby用語集/継承>))
  も((<Ruby用語集/Mix-in>))もクラスにメソッドを追加するこ
  とだが、(({undef}))を使えばメソッドを取り除くことがで
...
: オーバーライド
: Override
  ((<Ruby用語集/再定義>))のこと。スーパークラスまた
  は((<Ruby用語集/include>))しているモジュールで定義され
  ているメソッドと同じ名前のメソッドを定義すること。オーバーラ
  イドした上位のメソッドは
  ((<Ruby用語集/super>))を使って呼び出すこと
  ができる。
...
: Object
  ...
  人がいる。困ったものだ。((<Ruby用語集/カプセル化>))、
  ((<Ruby用語集/抽象データ型>))参照。

: オブジェクト指向
  オブジェクトを基本にした((<Ruby用語集/パラダイム>))。
...
: Class Method
  ...
  ((<Ruby用語集/特異メソッド>))とがあるが、そんな
  ことは大した問題ではない。
  クラスメソッド内での(({self}))はクラスであるので勘違いしないように。
...
: Dictionary
  ...
  ((<Ruby用語集/ハッシュ>))の別名。オブジェクト指向の起源と
  も呼べる((<Ruby用語集/Smalltalk>))においてハッシュに
...
: Initialize
  ...
  メソッドを再定義する。クラスのメソッド
  ((<Class#new|Class>))のデフォルトの
  定義は新たに生成したインスタンスに対して、
  (({initialize}))を実行する。(({new}))への
  引数はそのまま(({initialize}))に渡される。また、
  (({new}))がブロックとともに呼び出された時には
  (({initialize}))にそのブロックがそのまま与えられる。

  ということは((<Class#new|Class>))
  を再定義する必要はないはずだ。
...
: スクリプト言語
: Script Language
  ((<Ruby用語集/スクリプト>))に従ってバッチ処理を行うイン
  タープリタのこと。人間も台本を読むという点においてスクリプト
  言語である。

: 即値
: Immediate Value
  ((<Ruby用語集/参照>))ではなく、実際の値が変数に格納さ
...
: 定数
: Constant
  一度定義したら値を変えることができない((<Ruby用語集/変数>))。
  でも、この定義は矛盾しているなあ。
...
: Dynamic Binding
  ...
  オブジェクト指向の要件のひとつ。
  Rubyでは((<Ruby用語集/変数>))に型が無いので動的結合は必然である。
...
: Singleton Method
  ...
  * ((<Kernel#clone|Object>))した場合

  * サブクラスを作った場合

  特異メソッドで元のクラスのメソッドをオーバーライドした場合は
  もとのメソッドは((<メソッド呼び出し/super>))で呼び
  出すことができる。
...
: Top Level
  ...
  トップレベルの ((<Ruby用語集/self>)) は ((<Ruby用語集/main>)) を指す。
...
: バイトオーダー
: Byte Order
  (({0x01020304}))という4バイトデータを(({1,2,3,4}))
  と配置するか、(({4,3,2,1}))と配置するかということ。前
  者を((<Ruby用語集/ビッグエンディアン>))、後者を
  ((<Ruby用語集/リトルエンディアン>))と呼ぶ。どちらが
  良いかという論争は時のはじめから続いていてまだ結論が出ていない。

: 破壊的
: Destructive
  ((<String#chop!|String>)),
  ((<Array#concat|Array>)) などの
...
: ブロック付きメソッド呼び出し
  ...
  ブロック付きメソッドの中では((<メソッド呼び出し/yield>))を使って
  ブロックを実行することができる。
...
: Variable
  ...
  ((<Ruby用語集/グローバル変数>))、
  ((<Ruby用語集/ローカル変数>))、
  ((<Ruby用語集/インスタンス変数>))がある。
  それと((<Ruby用語集/定数>))は値を変えることができない
...
: 多態, Polymorphism
  ...
  Rubyでは((<Ruby用語集/レシーバ>))のオブジェクトに応じ
...
: メソッド
: Method
  ((<Ruby用語集/オブジェクト>))に対する操作。操作対象のオ
  ブジェクト(((<Ruby用語集/レシーバ>)))は
  ((<Ruby用語集/self>))で参照できる。
  Rubyの場合((<Ruby用語集/ビルトインクラス>))のオブ
...
: Receiver
  ...
  側にあるもの。メソッド内では
  ((<Ruby用語集/self>))で参照できる。レシーバ
  の((<Ruby用語集/インスタンス変数>))は
  (({@変数名}))という形式でアクセスできる。

: 連想配列
: Associative Array
  ((<Ruby用語集/ハッシュ>))の別名。ハッシュが任意のキーから
