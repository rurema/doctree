...
リファレンスは英語で書かれています。似たようなものに ReFe があります。

=== インストール

ソースから Ruby をインストールしている場合は、
デフォルトでインストールされているはずです。
特に以下のように実行すると必要なものが全てインストールされます。

  $ make install-doc

ソースからでなくパッケージからインストールしている場合は、
各パッケージに依存します。

=== 使い方

クラス名やモジュール名の完全な文字列を入力しなくても、以下のように
リファレンスを引くことができます。当てはまる候補が複数ある場合は、
当てはまる候補のリストを表示します。
