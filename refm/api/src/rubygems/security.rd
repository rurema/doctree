...
: HighSecurity
  ...
  悪意のあるユーザは、なんの手段もも持っていない。悪意のあるユーザは、署名を
...
さて、4 つの信頼された証明書(alf@seattle, bob@portland, pabs@dc, tomecope@dc)
がありますが、実際にはユーザは、一つの証明書("rubygems@rubyforge.org")を信頼
するだけでいいのです。以下にそれがどのように働くか説明します。

私は "al@seatle" が署名した "Alf2000-Ruby-0.1.0.gem" をインストールします。
私は "al@seatle" なんて聞いたこともありませんが、彼の証明書は "seattle.rb@zenspider.com"
...
できるので "al@seatle" によって署名されたパッケージを信頼するのにものすごく便利だということです。
...
発行された証明書の持ち主 (このケースの場合 "alf@seatle") は自分のパッケージに署名するために
この証明書を使用することができます。ところで、みんなに彼の新しい素敵な署名済みの証明書を
知らせるには、"alf@seatle" は彼の gemspec を以下のように変更する必要があります。
