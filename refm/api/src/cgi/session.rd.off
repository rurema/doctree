...
セッションとは、HTTP の一連のリクエストとレスポンスが属するべきコンテクスト(状況)の
ことをいいます。セッション管理には従来通り、[[lib:cgi]] ライブラリが提供するクッキーを使用しても
いいですが、この cgi/session を使用した方がよりわかりやすいでしょう。
セッション情報は Hash ライクなインターフェースです。

セッションはセッション ID とプログラムが記録したセッション情報から構成されます。
デフォルトでは CGI::Session::FileStore が使用され、記録できるのは文字列のみです。

セッション情報は CGI::Session::FileStore か CGI::Session::PStore を使用した場合は
サーバのローカルファイルに記録され、次回のリクエスト時に利用されます。
デフォルトでは明示的に操作を行なわなくてもプログラム終了時にセッション情報は
ファイルに保存されます。セッション毎に新しいファイルが作成されます。

クライアントにはセッション情報に対応するセッション ID をクッキーあるいは form の hidden
input として渡すことになります。クッキーはデフォルトでは expires が指定されていない
ために、ブラウザを終了した時点で消滅します。
...
ヘッダ出力は [[m:CGI#out]]、[[m:CGI#header]] を使っている限り、通常通りで構いません。
...
#@if (version >= "1.8.2")
((<ruby 1.8.2 feature>)): セッション情報ファイルは 0600 で作成されるようになりました。
#@end

=== [[m:CGI::HtmlExtension#form]] の出力
...
=== 使用例
ただ、名前を入力するとあいさつをするだけのつまらない CGI スクリプト。
...
=== 参考URL
* [[m:URL:http:#/www.shugo.net/article/webdb2/#label:13]]
* [[m:URL:http:#/www.modruby.net/doc/faq.ja.jis.html#label-13]]
* [[unknown:rdoc|URL:http://www.ruby-doc.org/stdlib/libdoc/cgi/rdoc/index.html]]
