#@if (version >= "1.9.0")
UNIX Borne シェルの単語分割規則に従った文字列割機能と文字列エスケープ
機能を提供します。
...
#@if (version >= "1.9.0")
UNIX Borne シェルの単語分割規則に従った文字列分割と文字列エスケープ
を行うモジュールです。
...
#@if (version >= "1.9.0")
Borne シェルの単語分割規則に従った空白区切りの単語分割を行い、
単語 (文字列) の配列を返します。
#@else
...
文字列を Borne シェルのコマンドライン中で安全に使えるようにエスケープします。
...
配列の各要素である文字列に対して、Borne シェルのコマンドライン中で安全に
