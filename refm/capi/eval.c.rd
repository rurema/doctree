...
: static VALUE rb_call(VALUE klass, VALUE recv, ID mid, int argc, const VALUE *argv, int scope)
    ...
    ((|scope|)) は以下のように呼び出しの形式を示します。
      ...
      * 0: obj.method()   (private/protected メソッドを呼べない)
      * 1: method()       (private/protected メソッドも呼べる)
      * 2: method_or_lvar (1とはエラーメッセージが変わる)
      * 3: super
...
: void rb_jump_tag(int tag)
    ...
    ((|tag|))には上記関数の引数で受け取った((|state|))を指定します。

: void rb_load(VALUE fname, int wrap)
    参考: [ruby-list:21651]

    ((<load|組込み関数>))の低レベルインタフェースです。Rubyスクリプ
    トが格納されたファイル((|fname|)) をロードします。

    引数((|wrap|))が、non-zeroなら無名のモジュールを生成して、ロー
...
: void rb_thread_schedule(void)
    ...
    see also: rb_thread_wait_fd, rb_thread_wait_for
