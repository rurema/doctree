...
[[ref:B]]　[[ref:C]]　[[ref:D]]　[[ref:E]]　[[ref:F]]　[[ref:G]]　[[ref:I]]　[[ref:J]]　[[ref:M]]　[[ref:N]]　[[ref:O]]　[[ref:R]]　[[ref:S]]　[[ref:Y]]
...
: dRuby
  ...
  ための仕組み。
  何らかの通信手段を介して異なるプロセスやマシンの間でオブジェクトにアクセスする
  ことができる。

===[a:E] E

: enjoy programming
  Matz が Ruby の仕様策定において重視していること。
...
: Opal
  Ruby のコードを JavaScript に変換するソフトウエア。
...
: アリティー
: arity
  メソッドやそれをオブジェクト化した [[c:Method]] オブジェクトの仮引数の引数、および
...
: operator
  ...
  おける & も演算子である。
  defnined? のように、見た目がメソッドのような演算子もある。

  参照：[[d:spec/operator]]

: 汚染
: taint
  Ruby のオブジェクトは、汚染されているか否かという状態を持つ。
...
: object
  ...
  ただし、メソッド、ブロックはそれぞれオブジェクト化することができる。
...
: object id
  ...
  オブジェクトが同一であることとオブジェクト ID が一致していることは同値である。
...
: coding convention
  ...
  スペースや改行の入れ方、1 行の最大字数、コメントの記法、プログラム要素の配列順、といった
...
: Boolean value
  ...
  Ruby で、ある値を真偽値と呼ぶのはその値の真偽性を利用するときであって、
  その値の型・種類には関わりない。
...
: superclass
  ...
  「スーパークラス」と呼ぶ場合もあるが、この場合は「先祖クラス」という
  用語が使える。
...
: magic comment
  ...
  スクリプトエンコーディングを示すものと、文字列リテラルを凍結するかどうか
  を指定するものとの二種類がある。
...
: ユニットテスト
: unit test
  メソッドなど、プログラムの小さな単位で行われるソフトウエアテスト。
  単体テストともいう。
...
: reserved word
  ...
  変数・定数・クラス・メソッドなどの名前として自由に使えない文字列。
