...
= メソッド呼び出し
  ...
  * [[unknown:メソッド呼び出し/ブロック付きメソッド呼び出し]]
...
`::' を使わなければいけません(例: [[unknown:Math::PI|Math/PI]],
[[unknown:Errno::ERANGE|Errno::EXXX/Errno]])。逆に、

   Klass::Foo

とした場合、常に定数と見なされるという制限があります。
`::' を、クラスメソッ
ド呼び出しに使うという流儀がありますが((-あらい 2002-04-21: このマニュ
...
個人的な考えからです-))、この点は要注意です。
大文字で始まるメソッド名を使用する場合は
...
ブロック付きメソッドを自分で定義するには [[unknown:メソッド呼び出し/yield]] 式を使います。
...
((<ruby 1.7 feature>)):
version 1.7 では、to_proc メソッドを持つオブジェクトならば、`&'
...
[[unknown:制御構造/break]] により中断された場合は nil を返します。
((<ruby 1.7 feature>)): break に引数を指定した場合はその値が
ブロック付きメソッドの戻り値になります。
...
[[unknown:メソッド呼び出し/イテレータ]]を定義するために
[[unknown:クラス／メソッドの定義/メソッド定義]]内で使用します。
...
ブロック引数の代入は[[unknown:演算子式/多重代入]]と同じルールで行われます。
また yield を実行したメソッドにブロックが渡されていない
(イテレータではない) 時は例外 [[c:LocalJumpError]] が発生します。

yield はブロック内で最後に評価した式の値を返します。また、
[[unknown:制御構造/next]] によりブロックの実行が中断された場合は nil
を返します。

((<ruby 1.7 feature>)):
next に引数を指定した場合はその値が yield の戻り値になりま
す。
...
((-[[unknown:ruby-dev:20358]], [[unknown:ruby-dev:20409]]-))
