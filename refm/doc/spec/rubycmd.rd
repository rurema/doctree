...
= Rubyの起動
  ...
  * [[unknown:Rubyの起動/コマンドラインオプション]]
  * [[unknown:Rubyの起動/インタプリタ行の解釈]]
...
ここで、option は後述の[[unknown:Rubyの起動/コマンドラインオプション]]
...
が行われます。詳細は後述の[[unknown:Rubyの起動/インタプリタ行の解釈]] を参照
してください

argument に指定した文字列は組み込み定数 [[m:Kernel::ARGV]] の初
期値として設定されます。標準のシェルがワイルドカードを展開しない環境
([[c:Win32]])では、Ruby インタプリタが自前でワイルドカードを展開して
