...
Ruby 1.8 からの移行措置として、コマンドオプション -K と -E を適切に指定すれば
Ruby 1.8 用に書かれたスクリプトもできる限り動くような仕様になっています。
ただし Ruby 1.9 には多言語化以外にも多くの非互換性が存在するので必ずしも動くとは限りません。

: -K オプション
  スクリプトを書くのに使われているエンコーディングを指定します。
  本来はマジックコメントで指定すべきです。

  -K オプションは Encoding.default_external にも影響を与えます。

: -E オプション
  テキストデータを保存するのに使っているエンコーディングを指定します。
...
例:
  ...
  スクリプトを EUC-JP で書いていて、扱うテキストが UTF-8 の場合。  => コマンドオプションに -Ke -E utf-8 を指定。

  スクリプトを US-ASCII で書いていて、扱うテキストが UTF-8 の場合。=> コマンドオプションに -E utf-8 を指定。
