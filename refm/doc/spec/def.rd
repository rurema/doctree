...
[[ref:d:spec/call#yield]] を呼び出すことです。
...
またメソッド実行時の例外を捕捉するために [[ref:d:spec/control#begin]] 式と同様
...
メソッドの戻り値は [[ref:d:spec/control#return]] に渡した値です。return
が呼び出されなかった場合は、body の最後の式の値を返します。

body の最後の式が([[ref:d:spec/control#while]]など)値を返さない式の場合は
nil を返します。
...
味します。
添付ライブラリの [[lib:importenv]] はこのことを利用して[[unknown:組み込み変数]]
に英語名をつけます。
...
[[ref:d:spec/call#super]] の実行が可能なら真(文字列 "super")を返します。
