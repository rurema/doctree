...
[[unknown:クラス／メソッドの定義/呼び出し制限]]のデフォルトが異なること以外
...
(Ruby では、モジュールもオブジェクトの一つで [[c:Module]] クラスのイン
スタンスです)。
...
など [[unknown:演算子式]] を参照)も指定できます([[unknown:クラス／メソッドの定義/演算子式の定義]]参照)。
...
[[unknown:メソッド呼び出し/イテレータ]]を定義する方法の一つです。(イテレータを
定義する代表的な方法は [[unknown:メソッド呼び出し/yield]] を呼び出すことです。
他に [[m:Proc.new]]/[[m:Kernel.#proc]] を使う方法などもありま
す。)
...
またメソッド実行時の例外を捕捉するために [[unknown:制御構造/begin]] 式と同様
...
[[unknown:演算子式]]において、「再定義可能な演算子」に分類された演算子の実装
...
[[unknown:ruby-talk:10006]],[[unknown:ruby-dev:12829]]にあります-))。

=== メソッド定義のネスト

[[unknown:クラス／メソッドの定義/特異メソッド定義]]を除くメソッド定義式は
ネストできません。
...
メソッドの戻り値は [[unknown:制御構造/return]] に渡した値です。return
が呼び出されなかった場合は、body の最後の式の値を返します。

body の最後の式が([[unknown:制御構造/while]]など)値を返さない式の場合は
nil を返します。
...
[[m:Module#public]]、[[m:Module#private]]、
[[m:Module#protected]] を用いて変更できます。ただし
[[m:Object#initialize]] という名前のメソッドと
[[m:Object#initialize_copy]](((<ruby 1.8 feature>)))という名前のメソッ
...
味します。
添付ライブラリの [[unknown:importenv]] はこのことを利用して[[unknown:組み込み変数]]
に英語名をつけます。
...
([[unknown:組み込み変数]]を参照)を再定義すると動作に支障を来す場合が
あります。
...
[[c:NameError]] を発生させます。
(一方、メソッド
[[m:Module#remove_method]] は、関係を取
り除くだけです。この違いは重要です)。

aliasによる別名定義と
undefによる定義取り消しによってクラスのインタフェー
...
[[unknown:メソッド呼び出し/super]] の実行が可能なら真(文字列 "super")を返します。
