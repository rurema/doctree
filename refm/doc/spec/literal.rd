...
? 表現では全ての[[unknown:リテラル/バックスラッシュ記法]]が有効です。
...
ダブルクォートで囲まれた文字列では[[unknown:リテラル/バックスラッシュ記法]]
と[[unknown:リテラル/式展開]](後述)が有効になります。シングルクォートで囲ま
れた文字列では、\\(バックスラッシュそのもの)と
\'(シングルクォート)、行末の\(改行を無視します)
を除いて文字列の中身の解釈は行われません。
((-((<ruby 1.8 feature>)): 1.8.1(?) 以降ではシングルクォートで囲まれた文
字列では行末の\は \ そのものとして解釈されます。[[unknown:ruby-dev:21339]]-))
...
[[unknown:リテラル/%記法]] による別形式の文字列表現もあります。
...
トで囲まれた文字列と同様に[[unknown:リテラル/バックスラッシュ記法]]
の解釈と[[unknown:リテラル/式展開]]
が行なわれた後、コマンドとして実行され、その標準出力が文字列
として与えられます。コマンドは評価されるたびに実行されます。
コマンドの終了ステータスを得るには、[[m:$?]] を
参照します。

[[unknown:リテラル/%記法]] による別形式のコマンド出力もあります。
...
普通の[[unknown:リテラル/文字列リテラル]]はデリミタ(", ', ` など)で囲まれた
...
メタ文字については[[unknown:正規表現]]を参照してください。
...
[[unknown:リテラル/%記法]] による別形式の正規表現も指定できます。

正規表現の中では文字列と同じ[[unknown:リテラル/バックスラッシュ記法]]や
[[unknown:リテラル/式展開]]も有効です。

正規表現リテラルはその中に[[unknown:リテラル/式展開]]を含まなければ、何度評
価されても同一の正規表現オブジェクトを返します。
[[unknown:リテラル/式展開]]を含む場合は評価のたびに(式展開の結果を元に)正規
...
要素が文字列リテラルの場合に限り、[[unknown:リテラル/%記法]] による別形式の
配列表現も指定できます。
...
ド呼び出しと[[unknown:リテラル/配列式]]では、要素全体がハッシュの場合に限り、
{, }を省略することができます。
...
:"..." の形式は、[[unknown:リテラル/バックスラッシュ記法]]や
[[unknown:リテラル/式展開]]が有効です。
