...
`(({::}))' を使わなければいけません(例: ((<Math::PI|Math/PI>)),
((<Errno::ERANGE|Errno::EXXX/Errno>)))。逆に、
...
渡されます((-展開は(({to_a}))を経由して行なわれます。-))。つまり:
...
ブロック付きメソッドを自分で定義するには ((<メソッド呼び出し/yield>)) 式を使います。
...
((<制御構造/break>)) により中断された場合は (({nil})) を返します。
...
((<メソッド呼び出し/イテレータ>))を定義するために
((<クラス／メソッドの定義/メソッド定義>))内で使用します。
