...
((<ruby 1.7 feature>)): version 1.7 では、rescue/ensure 節を指定で
きます。

クラス定義は、識別子で指定した定数へのクラスの代入になります
(Ruby では、クラスもオブジェクトの一つで ((<Class>))クラスの
インスタンスです)。
...
((<クラス／メソッドの定義/呼び出し制限>))のデフォルトが異なること以外
...
((<ruby 1.8 feature>)): クラス Foo が既に定義されていれば、以下の書き
方もできます。
...
(Ruby では、モジュールもオブジェクトの一つで ((<Module>)) クラスのイン
スタンスです)。
...
など ((<演算子式>)) を参照)も指定できます(((<クラス／メソッドの定義/演算子式の定義>))参照)。
...
最後の仮引数の直前に (({*})) がある場合には残りの実引数はみな配列とし
てこの引数に格納されます。
可変長引数などと呼ばれる機能です。
...
((<メソッド呼び出し/イテレータ>))を定義する方法の一つです。(イテレータを
定義する代表的な方法は ((<メソッド呼び出し/yield>)) を呼び出すことです。
他に ((<Proc/Proc.new>))/((<組み込み関数/proc>)) を使う方法などもありま
す。)
ブロックが与えられなかった場合のブロック引数の値は(({nil}))です。
...
またメソッド実行時の例外を捕捉するために ((<制御構造/begin>)) 式と同様
の(({rescue})), (({else})), (({ensure})) 節を指定できます。
...
((<演算子式>))において、「再定義可能な演算子」に分類された演算子の実装
...
マンド実行の挙動に不具合がある場合などに利用できます((- 実際の応用例が
((<ruby-talk:10006>)),((<ruby-dev:12829>))にあります-))。

=== メソッド定義のネスト

((<クラス／メソッドの定義/特異メソッド定義>))を除くメソッド定義式は
ネストできません。
...
version 1.6 以前は、同じことを行うのに ((<Object/instance_eval>)) を使
...
または、以下のように書くこともできます。((-1.6 以前は、def 式内の def 式が
...
引数のデフォルト式も含め、すべてそのメソッドのコンテキストで評価されま
す。

メソッドの戻り値は ((<制御構造/return>)) に渡した値です。(({return})) 
が呼び出されなかった場合は、body の最後の式の値を返します。

body の最後の式が(((<制御構造/while>))など)値を返さない式の場合は 
(({nil})) を返します。
...
(({extend})) については、((<Object#extend|Object/extend>)) を参照して
ください。
...
* (({protected})) に設定されたメソッドは、そのメソッドを持つオブジェクト
  のメソッド定義式内でなければ呼び出せません。
...
デフォルトでは (({def})) 式がクラス定義の外(トップレベル)にあれば 
(({private}))、クラス定義の中にあれば (({public})) に定義します。これは
((<Module#public|Module/public>))、((<Module#private|Module/private>))、
((<Module#protected|Module/protected>)) を用いて変更できます。ただし 
((<Object/initialize>)) という名前のメソッドと 
((<Object/initialize_copy>))(((<ruby 1.8 feature>)))という名前のメソッ
ドは定義する場所に関係なく常に (({private})) になります。
...
((-正確には、(({private})) には((*関数*))を定義する目的があるが、呼び
出し制限の目的でも(ここに挙げた制限があるにもかかわらず)
(({protected})) よりは (({private})) が使われることの方が多いようだ-))
...
メソッドの定義内で別名を付けるには((<Module>))クラスのメソッド
((<Module#alias_method|Module/alias_method>))
を利用して下さい。
...
味します。
添付ライブラリの ((<importenv>)) はこのことを利用して((<組み込み変数>)) 
に英語名をつけます。
((<ruby 1.7 feature>)): version 1.6 では、グローバル変数の別名づけは、
...
(({$1})),(({$2})), ... には別名を付けることができません。ま
た、インタプリタに対して重要な意味のあるグローバル変数
(((<組み込み変数>))を参照)を再定義すると動作に支障を来す場合が
あります。
...
メソッドの定義内で定義を取り消すには((<Module>))クラスのメソッ
ド ((<Module#undef_method|Module/undef_method>)) を利用して下
さい。

(({undef})) のより正確な動作は、メソッド名とメソッド定義との
関係を取り除き、そのメソッド名を特殊な定義と関連づけます。この状態の
メソッドの呼び出しは例えスーパークラスに同名のメソッドがあっても例外 
((<NameError>)) を発生させます。
(一方、メソッド 
((<Module#remove_method|Module/remove_method>)) は、関係を取
り除くだけです。この違いは重要です)。
...
((<Module#remove_method|Module/remove_method>)) により削除さ
れたメソッドのいずれに対しても (({defined?})) は偽を返します。
...
((<組み込み関数/block_given?>)) と同様にメソッドがブロック付きで呼ばれたか
を判断する方法になります。

   defined? super

((<メソッド呼び出し/super>)) の実行が可能なら真(文字列 "super")を返します。
